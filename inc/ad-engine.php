...
/* ================================================================
 * CMP CONSENT TAG — InMobi Choice (TCF v2.2 / GPP)
 *
 * MUST be output as the first <script> in <head>, BEFORE GPT.
 * The __tcfapi stub runs synchronously so GPT detects the CMP
 * immediately. choice.js loads async — never blocks rendering.
 *
 * Called directly from header.php (not via wp_head) to guarantee
 * position before all other scripts.
 * ================================================================ */

/**
 * Output the InMobi Choice CMP consent tag.
 *
 * Only outputs when ads are enabled (respects the master toggle).
 * The inline stub creates __tcfapi, __gpp, and __uspapi before
 * any ad scripts load. choice.js then takes over async.
 */
function pinlightning_output_cmp_tag() {
	$s = pl_ad_settings();
	if ( ! $s['enabled'] ) {
		return;
	}
	// Skip in admin / REST / AJAX contexts.
...
.concat('/choice/', 'M65A7dGLumC_E', '/', host, '/choice.js?tag_version=V3');
var uspTries = 0;
var uspTriesLimit = 3;
element.async = true;
...
var args = arguments;
if (!args.length || (args.length === 1 && typeof args[0] === 'string' && args[0] === 'queue')) {
return queue;
} else if (args.length === 1 && typeof args[0] === 'string' && args[0] === 'events') {
return events;
...
supportedAPIs: ['2:tcfeuv2', '5:tcfcav1', '6:uspv1', '7:usnatv1', '8:usca', '9:usva', '10:usco', '11:usut', '12:usct'],
cmpId: CMP_ID,
sectionList: [],
...
if (!('lastId' in addtl)) { addtl.lastId = 0; }
addtl.lastId++;
var lnr = args[1];
events.push({ id: addtl.lastId, callback: lnr, parameter: args[2] });
if (typeof lnr === 'function') {
lnr({
eventName: 'listenerRegistered',
listenerId: addtl.lastId,
data: true,
pingData: {
gppVersion: '1.1',
cmpStatus: 'stub',
...
} else if (args[0] === 'removeEventListener') {
for (var i = 0; i < events.length; i++) {
if (events[i] && events[i].id === args[1]) { events.splice(i, 1); break; }
}
if (typeof args[1] === 'function') { args[1](true); }
} else if (args[0] === 'hasSection') {
if (typeof args[1] === 'function') { args[1](false); }
} else if (args[0] === 'getSection' || args[0] === 'getField') {
...
window.__gpp.a = addtl;
}

window.addEventListener('message', function(evt) {
var msgIsString = typeof evt.data === 'string';
var json = {};
try {
if (msgIsString) { json = JSON.parse(evt.data); } else { json = evt.data; }
} catch (e) {}
var payload = json.__gppCall;
if (payload) {
window.__gpp(payload.command, function(retValue, success) {
var returnMsg = { __gppReturn: { returnValue: retValue, success: success, callId: payload.callId } };
if (msgIsString) { returnMsg = JSON.stringify(returnMsg); }
if (evt && evt.source && evt.source.postMessage) { evt.source.postMessage(returnMsg, '*'); }
}, 'parameter' in payload ? payload.parameter : null, 'version' in payload ? payload.version : '1.1');
}
}, false);
}
...
function makeUspStub() {
if (typeof window.__uspapi !== 'undefined') return;
function uspAPIHandler() {
var args = arguments;
if (typeof args[2] === 'function') {
args[2]({ uspString: '1---', version: 1 }, true);
}
}
window.__uspapi = uspAPIHandler;
var uspFrame = window;
while (uspFrame) {
try { if (uspFrame.frames['__uspapiLocator']) { return; } } catch (e) {}
if (uspFrame === window.top) { break; }
uspFrame = uspFrame.parent;
}
if (document.body) {
