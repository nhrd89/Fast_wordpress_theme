...
function pl_opt_ajax_save() {
	...
	if ( isset( $raw['layer2'] ) && is_array( $raw['layer2'] ) ) {
		...
		update_option( 'pl_ad_layer2_settings', array(
			...
			'minPixelSpacing'      => max( 500, min( 1000, (int) ( $l['minPixelSpacing'] ?? $d['minPixelSpacing'] ) ) ),
			...
			<div class="pl-opt-body">
				...
				<table class="form-table">
					...
					<tr>
						...
						<td>
							<label>Min spacing: <input type="number" id="opt_l2_minPixelSpacing" value="<?php echo (int) $l2['minPixelSpacing']; ?>" min="500" max="1000" class="small-text"> px</label><br>
	...
	(function(){
		...
		document.getElementById('opt_density').addEventListener('change', function() {
			var presets = {
				...
				normal:     { minPixelSpacing: 500, maxPixelSpacing: 1000, desktopMaxInView: 2, mobileMaxInView: 1, maxAdDensityPercent: 30, maxSlots: 20, predictiveSpeedCap: 300 },
				aggressive: { minPixelSpacing: 500, maxPixelSpacing: 800,  desktopMaxInView: 3, mobileMaxInView: 2, maxAdDensityPercent: 40, maxSlots: 20, predictiveSpeedCap: 400 },
			};
