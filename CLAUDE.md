...
- **fix: diverse categories** — Trending Now queries 20 posts, picks first 4 with unique primary categories. Latest Stories queries 24, caps 2 per category for variety, backfills if too strict. Most Loved fallback chain: comment_count → rand (30 days) → rand (all, ignoring shown_ids). Footer replaced with `get_footer()` + `.ee-home .pl-footer` CSS overrides for emerald colors.
