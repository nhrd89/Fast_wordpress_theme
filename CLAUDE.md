...
- **template-emerald-editorial.php** — 5-section layout: hero (sticky post preferred, eager+fetchpriority), trending sidebar (4 recent posts by date), category circles (transient-cached), latest grid (6 posts + Load More), newsletter CTA (inline JS, `/pl/v1/subscribe` endpoint). Uses `get_footer()` for shared footer with emerald CSS overrides. Post deduplication via `$shown_ids` across all queries. All queries use `no_found_rows => true`.
...
- **fix: trending + load more** — Trending simplified to 4 recent by date (fallback: exclude hero only → no exclusions). Most Loved section removed entirely (too few posts). Load More button after Latest Stories: fetches `/wp-json/wp/v2/posts?_embed`, builds card HTML matching PHP markup, hides when `X-WP-TotalPages` reached, excludes `$shown_ids` via `data-exclude`. Footer: `get_footer()` + `.ee-home .pl-footer` CSS overrides.
